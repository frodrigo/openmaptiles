...
RETURNS TABLE(osm_id bigint, geometry geometry, name text, name_en text, name_de text, tags hstore, superclass text, class text, subclass text, zoom integer, priority integer, style text, agg_stop integer, layer integer, level integer, indoor integer, rank int) AS $$
    SELECT osm_id_hash AS osm_id, geometry, NULLIF(name, '') AS name,
        ...
        (poi_tourism_class(mapping_key, subclass, tags)).priority AS priority,
