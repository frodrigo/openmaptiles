    ...
    FROM (
      ...
      row_number() OVER (
        ...
        FROM (
            ...
            UNION ALL
            SELECT osm_id, ST_PointOnSurface(geometry) AS geometry, name, name_en, name_de, tags, place, population, capital, rank FROM osm_city_polygon WHERE geometry && bbox
        ) AS osm_city_point
        WHERE
