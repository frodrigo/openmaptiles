...
CREATE INDEX IF NOT EXISTS osm_transportation_name_linestring_ref_idx ON osm_transportation_name_linestring(ref);
...
BEGIN
    ...
    THEN
        INSERT INTO transportation_name.name_changes(is_old, osm_id, name, name_en, name_de, ref, highway, construction, "level", layer, indoor, network_type)
        VALUES (true, OLD.osm_id, OLD.name, OLD.name_en, OLD.name_de, OLD.ref, OLD.highway, OLD.construction, OLD."level", OLD.layer, OLD.indoor, OLD.network_type);
    END IF;
    ...
    THEN
        INSERT INTO transportation_name.name_changes(is_old, osm_id, name, name_en, name_de, ref, highway, construction, "level", layer, indoor, network_type)
        VALUES (false, NEW.osm_id, NEW.name, NEW.name_en, NEW.name_de, NEW.ref, NEW.highway, NEW.construction, NEW."level", NEW.layer, NEW.indoor, NEW.network_type);
    END IF;
    RETURN NULL;
END;
$$ language plpgsql;
...
CREATE OR REPLACE FUNCTION transportation_name.refresh_name() RETURNS trigger AS
  ...
  BEGIN
    ...
    CREATE TEMP TABLE name_changes AS
    ...
    FROM
     ((
        SELECT DISTINCT ON (osm_id)
            *,
            coalesce(name, '') || coalesce(ref, '') AS idx_key
        FROM
            transportation_name.name_changes
        WHERE
            is_old
        ORDER BY
            osm_id,
    ...
    USING name_changes AS c
    WHERE
        c.name IS NULL AND -- IF name IS NULL then ref IS NOT NULL
        n.ref = c.ref AND
        n.name IS NOT DISTINCT FROM c.name AND n.name_en IS NOT DISTINCT FROM c.name_en AND n.name_de IS NOT DISTINCT FROM c.name_de AND n.ref IS NOT DISTINCT FROM c.ref AND n.highway IS NOT DISTINCT FROM c.highway AND n.construction IS NOT DISTINCT FROM c.construction AND n.network IS NOT DISTINCT FROM c.network_type
    ;
    ...
    FROM (
    SELECT
        ST_LineMerge(ST_Collect(n.geometry)) AS geometry,
        n.name,
        n.name_en,
        n.name_de,
    ...
    FROM osm_transportation_name_network AS n
      JOIN name_changes AS c ON
        c.name IS NOT NULL AND
        n.name = c.name AND
        n.name IS NOT DISTINCT FROM c.name AND n.name_en IS NOT DISTINCT FROM c.name_en AND n.name_de IS NOT DISTINCT FROM c.name_de AND n.ref IS NOT DISTINCT FROM c.ref AND n.highway IS NOT DISTINCT FROM c.highway AND n.construction IS NOT DISTINCT FROM c.construction AND n.network_type IS NOT DISTINCT FROM c.network_type
