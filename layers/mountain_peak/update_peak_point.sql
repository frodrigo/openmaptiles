...
CREATE OR REPLACE FUNCTION update_osm_peak_point(new_osm_id bigint) RETURNS VOID AS $$
    ...
    WHERE
        (new_osm_id IS NULL OR osm_id = new_osm_id) AND
        COALESCE(tags->'name:latin', tags->'name:nonlatin', tags->'name_int') IS NULL AND
        tags != update_tags(tags, geometry)
$$ LANGUAGE sql;
...
CREATE CONSTRAINT TRIGGER trigger_update_point
    AFTER INSERT OR UPDATE ON osm_peak_point
    INITIALLY DEFERRED
    FOR EACH ROW
