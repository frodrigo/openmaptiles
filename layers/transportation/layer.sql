CREATE TABLE IF NOT EXISTS osm_route_network AS
SELECT
    "type", member,
    (ARRAY['icn', 'ncn', 'rcn', 'lcn'])[min(array_position(ARRAY['icn', 'ncn', 'rcn', 'lcn'], network::text))] AS bicycle_network,
    (ARRAY['iwn', 'nwn', 'rwn', 'lwn'])[min(array_position(ARRAY['iwn', 'nwn', 'rwn', 'lwn'], network::text))] AS hiking_network
FROM
    osm_route_member
WHERE
    network != '' AND
    (
        (route = 'bicycle' AND network IN ('icn', 'ncn', 'rcn', 'lcn')) OR
        (route = 'hiking' AND network IN ('iwn', 'nwn', 'rwn', 'lwn'))
    )
GROUP BY
    "type", member;

CREATE INDEX IF NOT EXISTS osm_route_network_member_idx ON osm_route_network(member);
...
indoor INT, bicycle TEXT, foot TEXT, horse TEXT, mtb_scale TEXT, bicycle_network TEXT, hiking_network TEXT, surface TEXT) AS $$
    SELECT
        osm_id, geometry,
        CASE
        ...
        NULLIF(bicycle_network, '') AS bicycle_network,
        NULLIF(hiking_network, '') AS hiking_network,
    ...
    FROM (
        ...
        SELECT
            ...
            NULL::text AS bicycle_network,
            NULL::text AS hiking_network,
        ...
        SELECT
            ...
            END AS indoor,
            bicycle, foot, horse, mtb_scale,
            osm_route_network.bicycle_network,
            osm_route_network.hiking_network,
            surface_value(surface) AS "surface",
            z_order
        FROM osm_highway_linestring
            LEFT JOIN osm_route_network ON
                osm_route_network.member = osm_highway_linestring.osm_id
        WHERE NOT is_area AND (
            zoom_level = 12 AND (
                highway_class(highway, public_transport, construction) NOT IN ('track', 'path', 'minor')
                OR highway IN ('unclassified', 'residential', 'cycleway')
            ) AND man_made <> 'pier'
            OR zoom_level = 13
                AND (
        ...
        SELECT
            ...
            NULL::int AS layer, NULL::int AS level, NULL::boolean AS indoor,
            NULL as bicycle, NULL as foot, NULL as horse, NULL as mtb_scale,
            NULL AS bicycle_network,
            NULL AS hiking_network,
            NULL as surface, z_order
